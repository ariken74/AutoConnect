  ...
  else
  {
    ...
    else
    {
      ...
      if (_apConfig.principle == AC_PRINCIPLE_RSSI)
      {
        ...
        if ((cs = _loadCurrentCredential(reinterpret_cast<char *>(current.ssid), reinterpret_cast<char *>(current.username), reinterpret_cast<char *>(current.password), AC_PRINCIPLE_RSSI, false)))
        {
    ...
    if (cs)
    {
      ...
      if (!_rfAdHocBegin)
        if (strlen((char *)_credential.username) > 0)
        {
          cs = _credentialConnect() != WL_CONNECT_FAILED;
        }
        else
        {
    ...
    if (!cs && _apConfig.autoReconnect && !_rfAdHocBegin)
    {
      ...
      char username_c[sizeof(station_config_t::username) + sizeof('\0')];
      AC_DBG("autoReconnect");
      if ((cs = _loadCurrentCredential(ssid_c, username_c, password_c, _apConfig.principle, strlen(reinterpret_cast<const char *>(current.ssid)) > 0)))
      {
...
template <typename T>
wl_status_t AutoConnectCore<T>::_credentialConnect()
{
  ...
  char username_c[sizeof(station_config_t::username) + 1];
  *ssid_c = '\0';
  ...
  *username_c = '\0';
  ...
  Serial.println(_credential.authMode);
  if (_credential.authMode == WIFI_AUTH_WPA2_ENTERPRISE)
  {
    strncat(username_c, reinterpret_cast<const char *>(_credential.username), sizeof(username_c) - 1);
    log_i("WiFi.begin(%s%s%s%s) ch(%d)", ssid_c, strlen(username_c) ? "," : "dotclock", strlen(username_c) ? username_c : "", strlen(username_c) ? "," : "", strlen(password_c) ? password_c : "", (int)ch);
    return WiFi.begin(ssid_c, WPA2_AUTH_TTLS, "", username_c, password_c, nullptr, nullptr, nullptr, 0, nullptr, true);
  }
  else
  {
    log_i("WiFi.begin(%s%s%s) ch(%d)", ssid_c, strlen(password_c) ? "," : "", strlen(password_c) ? password_c : "", (int)ch);
    return WiFi.begin(ssid_c, password_c, ch);
  }
}
...
void AutoConnectCore<T>::handleRequest(void)
{
  ...
  if (_rfConnect)
  {
    ...
    _portalStatus &= ~AC_TIMEOUT;
    if (_credentialConnect() != WL_CONNECT_FAILED)
    {
      _portalStatus |= AC_INPROGRESS;
 ...
 */
template <typename T>
bool AutoConnectCore<T>::_loadCurrentCredential(char *ssid, char *username, char *password, const AC_PRINCIPLE_t principle, const bool excludeCurrent)
{
  bool rc;
  ...
  if ((rc = _loadAvailCredential(nullptr, principle, excludeCurrent)))
  {
    ...
    strcpy(username, reinterpret_cast<const char *>(_credential.username));
...
String AutoConnectCore<T>::_induceConnect(PageArgument &args)
{
  ...
  else
  {
    ...
    if (args.hasArg(String(F(AUTOCONNECT_PARAMID_USER))))
    {
      strncpy(reinterpret_cast<char *>(_credential.username), args.arg(String(F(AUTOCONNECT_PARAMID_USER))).c_str(), sizeof(_credential.username));
      Serial.println(args.arg(String(F(AUTOCONNECT_PARAMID_USER))).c_str());
    }

    if (args.hasArg(String(F(AUTOCONNECT_PARAMID_USER))))
    {
      _credential.authMode = WIFI_AUTH_WPA2_ENTERPRISE; // (wifi_auth_mode_t) args.arg(String(F(AUTOCONNECT_PARAMID_AUTH))).toInt();
    }
...
/**
 * Convert dBm to the wifi signal quality.
 * @param  wifi_auth_mode_t
 * @return security type string
 */
template <typename T>
String AutoConnectCore<T>::_toSecurityType(wifi_auth_mode_t authMode)
{
  switch (authMode)
  {
  case WIFI_AUTH_OPEN:
...
#else
  return (config.bssid[0] == 0x00) & !memcmp(&config.bssid[0], &config.bssid[1], sizeof(station_config_t::bssid) - 1) ? !strcmp(reinterpret_cast<const char *>(config.ssid), WiFi.SSID(item).c_str()) : !memcmp(config.bssid, WiFi.BSSID(item), sizeof(station_config_t::bssid));
#endif
}
